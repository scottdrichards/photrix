...
export const notifyUnauthorized = () => {
  // A public share view is already authenticated by its own URL token. If a
  // later ancillary request gets a 401 (for example from a deployment-specific
  // non-share endpoint), bouncing the guest into the login screen is worse than
  // leaving the shared view up. Initial share-token validation still happens in
  // initAuth(), which can reject an actually invalid link on first load.
